...
class Tags():
    ...
    def add_tag_to_session(self, session_id, tag, targets):
        tagged = self.tags.get(tag, [])
        new_tagged = set(tagged).union(set([session_id + '/' + t for t in targets]))
        self.tags[tag] = list(new_tagged)
